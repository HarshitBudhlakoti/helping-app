...
const AdminPage = ({ status }) => {
  ...
  const dData = JSON.parse(localStorage.getItem("ddata")) || [];
  ...
  const [doneData, setDoneData] = useState(dData);
  ...
  const doneItem = (n) => {
    ...
    setFinalData(d);
    localStorage.setItem("data", JSON.stringify(d));
    const dd = finalData.filter((_, i) => n === i);
    localStorage.setItem("ddata", JSON.stringify([...doneData, ...dd]));
    setDoneData([...doneData, ...dd]);
  };

  if (finalData[0] === undefined) {
    return (
      <div className="h-screen bg-[url('https://images.unsplash.com/photo-1464618663641-bbdd760ae84a?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Mnx8d2Vic2l0ZSUyMGJhY2tncm91bmR8ZW58MHx8MHx8fDA%3D&w=1000&q=80')] bg-cover bg-no-repeat">
        <Link to="/" className="fixed top-0 right-0 text-3xl m-4">
          <BsBackspaceFill />
        </Link>
        <div className="text-2xl font-bold text-red-500 text-center">Done list</div>
        <div className="p-5">
            ...
            {doneData.map((item, i) => {
              return (
                ...
                <tr className="border-2 solid border-black p-4 font-semibold">
                  ...
                  <div
                    className="px-4 py-1 bg-green-500 m-4 font-bold rounded-xl shadow-md shadow-black"
                  >
                    Completed
                  </div>
                </tr>
